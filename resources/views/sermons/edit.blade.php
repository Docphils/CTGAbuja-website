...
<main class="container mx-auto text-white py-12 px-4">
    ...
    <form action="{{ route('sermons.update', $sermonVideo->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" id="title" class="w-full rounded-lg px-4 py-2" value="{{ old('title', $sermonVideo->title) }}">
        </div>
        <div class="mb-4">
            <label for="description" class="block text-lg font-semibold mb-2">Description:</label>
            <textarea name="description" id="description" rows="3" class="w-full rounded-lg px-4 py-2">{{ old('description', $sermonVideo->description) }}</textarea>
        </div>
        <div class="mb-4">
        ...
        <div class="mb-4">
            <button type="submit" class="inline-block px-6 py-3 text-lg font-semibold text-white bg-blue-600 rounded-lg shadow-md hover:bg-blue-700 transition duration-300 ease-in-out">Update</button>
        </div>
    </form>
</main>
